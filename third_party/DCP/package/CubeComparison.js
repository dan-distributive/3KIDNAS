module.declare(["./DataCube.js","./LikelihoodFunctions.js"], function (require, exports, module) {
'use strict';
...
//
// LikePoint is a function pointer in Fortran — he switch is threaded from 
// the fixture and resolved via selectLikelihood, no default.
//
...
const { threeDIndxCalc, flatIndxCalc } = require('./DataCube.js');
const { chi2Calc, chi2Calc_logElements, logChi2Calc } =
  require('./LikelihoodFunctions.js');

// Mirrors FittingOptionsInputs.f:51-56 — selects the likelihood routine
// from the fixture's likelihoodSwitch (1=chi2, 2=log-elements, 3=log-chi2).
function selectLikelihood(likelihoodSwitch) {
  switch (likelihoodSwitch) {
    case 1: return chi2Calc;
    case 2: return chi2Calc_logElements;
    case 3: return logChi2Calc;
    default:
      throw new Error(`Invalid likelihoodSwitch: ${likelihoodSwitch}`);
  }
}
...
// then computes the selected likelihood (chi², log-elements, or log-chi²) over those voxels.
//
// sigma: uniform noise level (scalar, same for all voxels)
// likelihoodSwitch: 1|2|3, selects the likelihood routine (see selectLikelihood).
//
// Returns chi2 as f32.
// ---------------------------------------------------------------------------
function cubeCompare(cube1, cube2, sigma, likelihoodSwitch) {
  const likePoint = selectLikelihood(likelihoodSwitch);
...
// Exports
// ---------------------------------------------------------------------------
module.exports = { cubeCompare, selectLikelihood };
...
if (require.main === module) {
  ...
  const chi2 = cubeCompare(c1, c2, sigma, 1);
  ...
  const chi2Zero = cubeCompare(c1, c1, sigma, 1);
