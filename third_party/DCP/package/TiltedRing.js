module.declare(["./Particle.js"], function (require, exports, module) {
'use strict';
...
//
// Pooled: reuses existing Particle objects from a prior call on this same
// Ring instead of allocating fresh every time (this runs once per ring per
// objective-function evaluation -- thousands of times per fit). Only grows
// the array, never shrinks it; every field any downstream code reads
// (angPos, pos, projectedPos[0:1], projectedVel[2], flux) is unconditionally
// overwritten by ring_ParticleGeneration on every call regardless of
// nParticles changing between calls, so reused objects carry no stale state
// into a read path. The three fields nothing ever reads (vel, projectedPos[2],
// projectedVel[0:1]) were verified unused across the whole port before
// relying on this.
// ---------------------------------------------------------------------------
function ring_ParticleAllocation(r) {
  if (!r.p) r.p = [];
  for (let i = r.p.length; i < r.nParticles; i++) {
    r.p[i] = new Particle();
  }
}
