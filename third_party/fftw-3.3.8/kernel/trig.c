...
#else
/* PATCHED (3KIDNAS FFTW3-port project): use the project's own vendored
 * fdlibm sin/cos (src/StandardMath/fdlibm_sin.c, fdlibm_cos.c) instead of
 * the platform libm. Different platforms' libm implementations can differ
 * from each other and from fdlibm by up to ~1 ULP (IEEE754 only mandates
 * correctly-rounded +,-,*,/,sqrt -- transcendentals are implementation
 * defined). This pipeline already made the same substitution for Fortran's
 * own SIN/COS intrinsics (see fdlibm_fortran_wrappers.c) for exactly this
 * reason; doing it here too removes the same risk from FFTW3's internal
 * twiddle-factor computation, and lets the planned JS port of real_cexp
 * (which will call the JS side's existing fdlibm.js) match bit-for-bit
 * regardless of host OS/libc. Resolved at final link time against
 * fdlibm_sin.o/fdlibm_cos.o (and their fdlibm_k_sin.o/fdlibm_k_cos.o/
 * fdlibm_rem_pio2_stub.o dependencies), which the Fortran build already
 * links into every binary that also links this library -- no new objects
 * need to be added to libfftw3.a itself.
 */
