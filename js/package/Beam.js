...
function allocate_Beam2D(b, nCubePixels) {
  ...
  // Left-to-right real4 evaluation, each op rounded separately: (2.*Pi)/2.355**2.,
  // then *Major, then *Minor -- NOT (2.*Pi/2.355**2.) done in double then a single
  // pre-grouped *(Major*Minor). The two orderings disagree on ~48% of inputs by 1 ULP.
  const beamAreaStep = f32(f32(f32(2.0) * Pi) / f32(FWHM_TO_SIGMA * FWHM_TO_SIGMA));
  b.beamAreaPixels = f32(
    f32(beamAreaStep * f32(b.beamMajorAxis)) * f32(b.beamMinorAxis)
  );

  // Beam area in world units
  // Fortran: BeamAreaPixels*abs(PixelSize(0))*abs(PixelSize(1)) -- same
  // left-to-right sequential rounding, not a pre-grouped abs*abs.
  b.beamAreaUnits = f32(
    f32(b.beamAreaPixels * f32(Math.abs(b.pixelSize[0])))
    * f32(Math.abs(b.pixelSize[1]))
  );
