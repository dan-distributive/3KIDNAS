...
c     BUG FIX: BeamPositionAngle (FITS 'BPA' keyword, always degrees per
c     the FITS standard) was never converted to radians here, unlike
c     BeamMajorAxis/BeamMinorAxis above -- every downstream use (kernel
c     rotation via cos/sin in CalculateBeamKernel.f, and re-serialization
c     via DataCubeOutputs.f's *180./Pi) assumes radians, so this degrees
c     value was being used directly as radians, and re-corrupted further
c     (an extra *180./Pi) on every write/read round-trip through a
c     bootstrap realization. FITS BPA is unambiguously degrees, so this
c     conversion is unconditional, not switch-gated (flagged by Dan, 2026).
      call DegreesToRadians(Beam%BeamPositionAngle
