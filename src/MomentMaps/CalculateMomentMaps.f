...
c      print*, shape(DC%Flux),idum
      ...
      do i=0,DC%DH%nPixels(0)-1
        do j=0,DC%DH%nPixels(1)-1
            ...
            do Moment=0,2
c               BUG FIX: must pass nChannels here, not nChannels-1 --
c               CalculateGeneralMoment uses this value both to size its
c               dummy arrays and to bound its summation loop, so passing
c               nChannels-1 silently dropped the last spectral channel
c               from every moment (flagged by Dan, 2026).
