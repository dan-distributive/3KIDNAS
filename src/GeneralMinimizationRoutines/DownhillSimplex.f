            ...
            do i=1,ndim+1
                if(i .ne. ilo) then
                    do j=1,ndim
                        ...
                        p(i,j)=psum(j)
                    enddo
c                   BUG FIX: funk recomputes chi2 at the shrunk position but
c                   the result was never stored into y(i), leaving y()
c                   inconsistent with the just-shrunk p(). Canonical
c                   Numerical Recipes amoeba stores it here.
                    call funk(psum,ytry)
                    y(i)=ytry
                endif
            enddo
