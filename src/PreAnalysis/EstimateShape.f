...
c                   EstimateRadialProfiles.f's BuildIniProfile fix, but
c                   here summed over every pixel in the map -- the
c                   dominant entry point for the shape/PA/inclination
c                   estimate's own noise (Dan, 2026).
                    aa2=aa2+Maps%Flux(i,j,0)*fd_cos(2.*theta)
                    bb2=bb2+Maps%Flux(i,j,0)*fd_sin(2.*theta)
                    nNonZero=nNonZero+1
                    FTot=FTot+Maps%Flux(i,j,0)
...
c                   CalculateFourerA2Moments's identical fix above.
                    aa1=aa1+Maps%Flux(i,j,1)*fd_cos(1.*theta)
                    bb1=bb1+Maps%Flux(i,j,1)*fd_sin(1.*theta)
                    nNonZero=nNonZero+1
                    FTot=FTot+Maps%Flux(i,j,0)
