...
c           BUG FIX: native cos()/sin() intrinsics here were not
c               guaranteed bit-identical to JS's Math.cos()/Math.sin()
c               (confirmed: a real 4-ULP disagreement traced all the way
c               to a bootstrap-realization simplex fork). Switched to the
c               shared fdlibm port already used everywhere else in this
c               pipeline for exactly this reason (Dan, 2026).
            xPrime=x*fd_cos(-Phi)-y*fd_sin(-Phi)
            yPrime=x*fd_sin(-Phi)+y*fd_cos(-Phi)
...
c           BUG FIX: native cos()/sin() intrinsics were not guaranteed
c               bit-identical to JS's Math.cos()/Math.sin() -- this exact
c               call site was traced (2026) as the entry point of a real
c               4-ULP Fortran/JS divergence that, after ~130 chained
c               optimizer evaluations, tipped a near-tied simplex
c               comparison and forked a whole bootstrap realization's fit
c               trajectory. Switched to the shared fdlibm port used
c               elsewhere in this pipeline (Dan, 2026).
        RadialProfile(1,i-1)=ProjectedProfile(1,i-1)*fd_cos(Incl)
c           For the velocity, normalize by the inclination to get a better guess
        RadialProfile(2,i-1)=ProjectedProfile(2,i-1)/fd_sin(Incl)
      enddo
...
c           BUG FIX: native sin() -> fd_sin() (see BuildIniProfile's
c               identical fix, this file, for why -- Dan, 2026).
      VProjLow=VSys-ProfilePt(2)*fd_sin(Incl)
      VProjHigh=VSys+ProfilePt(2)*fd_sin(Incl)
      minV=minval(DC%Channels)
      maxV=maxval(DC%Channels)
c      print*, "Proj Vel Check", VProjLow,VProjHigh
c     &      , minV, maxV
      if (VProjLow .lt. minV) then
        ProfilePt(2)=(VSys-minV)/fd_sin(Incl)
      endif
      if (VProjHigh .gt. maxV) then
        ProfilePt(2)=(maxV-VSys)/fd_sin(Incl)
      endif
...
c           For this ring loop to the right
     ...
     &                      ,RadialProfile(1:2,k)
                ...
                enddo
c           BUG FIX: only abort if the loop above found no fallback ring --
c           previously this stop ran unconditionally even after a
c           successful fill-in (flagged by Dan, 2026).
                if(.not. FoundFillIn) then
