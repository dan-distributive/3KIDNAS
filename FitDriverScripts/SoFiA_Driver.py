...
def RunSoFiA(GeneralDict,GalaxyDict):
    ...
    #   This only ever runs once per bootstrap realization (never for the
    #   single initial fit -- see the one caller in Bootstrap_Error_Analysis.
    #   GetBootstrapModel), and SoFiA's own native banner/progress output is
    #   verbose, so redirect it to a per-realization log file (ObjNameU is
    #   already unique per realization) instead of flooding the terminal
    #   nBootstraps times over.
    SoFiALogFile=GalaxyDict['SoFiAFolder']+GalaxyDict['ObjNameU']+"_SoFiALog.txt"
    RunCMD=GeneralDict['SoFiAExecPath']+" "+SoFiARunFile+" > "+SoFiALogFile+" 2>&1"
    os.system(RunCMD)
...
def LoadSoFiAOutput(ObjDict):
    ...
    if os.environ.get('WRKP_TRACE_DEBUG') == '1':
        print("TRACE SoFiA catalog ellMaj", WorkingDict['Ell_MajU'], "ellMin", WorkingDict['Ell_MinU'], "kin_pa", WorkingDict['PAU'], "maskVal", WorkingDict['MaxFluxIndx']+1, "x", WorkingDict['XU'], "y", WorkingDict['YU'], "z", WorkingDict['ZU'])
        print("TRACE geometry estimate paEst", PA, "incEst", IncApprox)
