...
def GetBootstrapModel(GeneralDict,GalaxyDict,Step):
    #   Per-realization native-call timing, in milliseconds, using the SAME
    #   common key names RunBootstrapsDCP/bootstrap-resample-launcher.js/
    #   bootstrap-fit-launcher.js use (resampleMs, sofiaMs, fitMs, totalMs) --
    #   plus fixtureFitMs/convolveMs/convolveCalls, which don't apply to this
    #   (local, native) path and are always None here, so run_both.js can
    #   read a single, consistently-shaped timings file from either side
    #   without special-casing which path produced it.
    TStart=time.time()
    Timing={'realizationIndex':Step,'resampleMs':None,'sofiaMs':None,
            'fixtureFitMs':None,'fitMs':None,'convolveMs':None,
            'convolveCalls':None,'totalMs':None}
    ...
    Timing['resampleMs']=(time.time()-t0)*1000.
    if os.environ.get('WRKP_TRACE_DEBUG') == '1':
        CubeHDU=fits.open(GalaxyDict['CubeNameU'])
        CubeData=CubeHDU[0].data
        print("TRACE resampled cube", "shape", CubeData.shape, "sum", np.nansum(CubeData),
              "min", np.nanmin(CubeData), "max", np.nanmax(CubeData),
    ...
    Timing['sofiaMs']=(time.time()-t0)*1000.
    ...
    if SoFiAGeo['SoFiASucces']:
        ...
        Timing['fitMs']=(time.time()-t0)*1000.
    ...
    Timing['totalMs']=(time.time()-TStart)*1000.
    CurrModel['Timing']=Timing
    #   One line per realization, matching the format bootstrap-realization-
    #   launcher.js's formatResultLine already prints for the js-local/js-dcp
    #   legs -- the Fortran binary/SoFiA's own verbose native output is
    #   redirected to per-realization log files instead of the terminal (see
    #   RunWRKP.RunWRKP / SoFiA_Driver.RunSoFiA), so this is the only line
    #   that should appear here per realization by default.
    print("  realization %d: converged=%s [total=%.0fms resample=%.0fms sofia=%.0fms fit=%.0fms]" % (
        Step, CurrModel.get('FITAchieved', False), Timing['totalMs'],
        Timing['resampleMs'] or 0, Timing['sofiaMs'] or 0, Timing['fitMs'] or 0))
    return CurrModel
