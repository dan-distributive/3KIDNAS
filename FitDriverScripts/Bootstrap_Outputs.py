...
def WriteBootstrappedFitOutputFile_Text(GalaxyDict):
    ...
    #   Defensive: this directory is created at the very start of GalaxyFit()
    #   and written into repeatedly during the bootstrap loop, but a real
    #   run has been observed reaching this point with it missing (cause
    #   not identified -- nothing in this codebase removes it). Cheap and
    #   idempotent either way.
    os.makedirs(GalaxyDict['WRKP_ResultsFolder'], exist_ok=True)
...
def SavePVDiagrams(GalaxyDict,GeneralDict):
    #   Defensive, see WriteBootstrappedFitOutputFile_Text's comment.
    os.makedirs(GalaxyDict['WRKP_ResultsFolder'], exist_ok=True)
...
def StoreBootstrappedModels_CSV(GalaxyDict,BootstrapModels):
    #   Defensive, see WriteBootstrappedFitOutputFile_Text's comment -- this
    #   is the file run_both.js's comparison depends on, so it matters most
    #   here that a missing directory doesn't silently skip this write.
    os.makedirs(GalaxyDict['WRKP_ResultsFolder'], exist_ok=True)
...
def StoreBootstrapTimings_JSON(GalaxyDict,Timings):
    """
    Writes per-realization timing breakdowns to <Obj>_BootstrapTimings.json,
    same folder/naming convention as StoreBootstrappedModels_CSV. Common
    schema for both the local (native) and DCP paths -- see
    Bootstrap_Error_Analysis.GetBootstrapModel (local) and
    RunBootstrapsDCP.RunBootstrapsDCP (DCP) for how each side's timing dict
    is built; fields that don't apply to a given path (e.g. convolveMs for
    local, fixtureFitMs for a realization whose SoFiA failed) are None
    rather than omitted, so run_both.js can rely on every entry having the
    same keys.
    """
    #   Defensive, see WriteBootstrappedFitOutputFile_Text's comment.
    os.makedirs(GalaxyDict['WRKP_ResultsFolder'], exist_ok=True)
    TimingsFile=GalaxyDict['WRKP_ResultsFolder']+GalaxyDict['ObjName']+"_BootstrapTimings.json"
    with open(TimingsFile,'w') as f:
