...
def WriteBootstrapFile(GeneralDict,GalaxyDict,Step):
    ...
    #   Random seed for the resampling RNG (BootstrapRuntimeInputs.f's
    #   `read(10,*) idum`). BootstrapSeed is opt-in (GeneralDict.get default 0):
    #   0 preserves today's behaviour exactly (BootstrapIn's own idum>=0 branch
    #   derives a time-based seed, same as when idum was left uninitialized).
    #   When a debug/run_both seed IS provided, derive a per-realization idum
    #   from it so each bootstrap Step gets its own distinct draw sequence --
    #   reusing one idum for every realization would make every "different"
    #   resample identical. Mirrored exactly in
    #   bootstrap-resample-launcher.js's resampleAndSoFiA for RNG matching.
    BootstrapSeed=GeneralDict.get('BootstrapSeed',0)
    if BootstrapSeed:
        Idum=-(abs(BootstrapSeed)+Step+1)
    else:
        Idum=0
    ExplanatoryStr="#    Get the random seed\n"
    f.write(ExplanatoryStr)
    f.write(str(Idum)+"\n")
